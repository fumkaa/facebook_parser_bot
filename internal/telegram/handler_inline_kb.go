	...
	case "confirm":
		...
		case Vehicles:
			go func() {
				...
				log.Printf("vehicles url: %s", url)
				msg := tgbotapi.NewMessage(message.Chat.ID, "Выберите тип транспортного средства:")
				...
				case All:
					...
					<-ChMake
					MakeBreak = true
					msg = tgbotapi.NewMessage(message.Chat.ID, "Выберите стиль кузова:")
					...
					<-ChExteriorColour
					msg = tgbotapi.NewMessage(message.Chat.ID, "Выберите цвет салона транспортного средства:")
					...
					<-ChInteriorColour
					msg = tgbotapi.NewMessage(message.Chat.ID, "Выберите тип коропки передач:")
		...
		case Propertyrentals:
			go func() {
				...
				url += fmt.Sprintf("?minPrice=%s&maxPrice=%s", minPrice, maxPrice)
				log.Printf("vehicles url: %s", url)
				msg := tgbotapi.NewMessage(message.Chat.ID, "Выберите количество комнат:")
				...
				<-ChRooms
				msg = tgbotapi.NewMessage(message.Chat.ID, "Выберите тип недвижимости, сдаваемой в аренду:")
		...
		case Free:
			go func() {
				...
				msg := tgbotapi.NewMessage(message.Chat.ID, "Вы выбрали категорию free")
				_, err := b.bot.Send(msg)
				if err != nil {
					log.Printf("error send message: %v", err)
				}
		...
		case Toys:
			go func() {
				...
				url += fmt.Sprintf("?minPrice=%s&maxPrice=%s", minPrice, maxPrice)
				log.Printf("vehicles url: %s", url)
				msg := tgbotapi.NewMessage(message.Chat.ID, "Выберите состояние предмета:")
		...
		case Home_improvements:
			go func() {
				...
				<-ChCondition
				msg = tgbotapi.NewMessage(message.Chat.ID, "Выберите бренд:")
		...
		case Groups:
			go func() {
				...
				msg := tgbotapi.NewMessage(message.Chat.ID, "Вы выбрали категорию groups")
				_, err := b.bot.Send(msg)
				if err != nil {
					log.Printf("error send message: %v", err)
				}
		...
		case All_listings:
			go func() {
				...
				msg := tgbotapi.NewMessage(message.Chat.ID, "Вы выбрали категорию all listings")
				_, err := b.bot.Send(msg)
				if err != nil {
					log.Printf("error send message: %v", err)
				}
